...
from app import app, db, EmailLog, Notification, NotificationSettings, EmailConfig, mail
...
with app.app_context():
    ...
    if email_config:
        ...
        print("   ⚠️  No hay configuración de email activa")
    ...
    if event_reg:
        ...
        print(f"   - event_registration: ⚠️  No configurada (por defecto habilitada)")
    
    if event_conf:
        print(f"   - event_confirmation: {'✅ Habilitada' if event_conf.enabled else '❌ Deshabilitada'}")
    else:
        print(f"   - event_confirmation: ⚠️  No configurada (por defecto habilitada)")
    
    # 5. Verificar si mail está configurado
    print("\n5. CONFIGURACIÓN DE FLASK-MAIL:")
    try:
        if hasattr(mail, 'app') and mail.app:
            print(f"   ✅ Flask-Mail está inicializado")
            ...
            print("   ⚠️  Flask-Mail no está inicializado correctamente")
