#!/usr/bin/env python3
"""
API para que los asesores gestionen sus slots.
Guards SaaS (módulo appointments) se registran en nodeone.core.features antes de register_blueprint.
"""
...
def _init_models():
    from app import Advisor, AppointmentAdvisor, AppointmentSlot, AppointmentType
    from nodeone.core.db import db

    return db, Advisor, AppointmentSlot, AppointmentType, AppointmentAdvisor
...
def list_slots():
    ...
    db, Advisor, AppointmentSlot, AppointmentType, _Aa = _init_models()
    ...
    query = AppointmentSlot.query.filter(AppointmentSlot.advisor_id == advisor.id)
...
def create_slot():
    """Crear un nuevo slot."""
    db, Advisor, AppointmentSlot, AppointmentType, AppointmentAdvisor = _init_models()
