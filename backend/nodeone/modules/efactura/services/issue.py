...
def find_accepted_fe_for_invoice(invoice_id: int, organization_id: int) -> ElectronicInvoiceDocument | None:
    return (
        ElectronicInvoiceDocument.query.filter(
            ElectronicInvoiceDocument.organization_id == int(organization_id),
            ElectronicInvoiceDocument.invoice_id == int(invoice_id),
            ElectronicInvoiceDocument.document_type == 'invoice',
            ElectronicInvoiceDocument.status == 'accepted',
        )
        .order_by(ElectronicInvoiceDocument.id.desc())
        .first()
    )
...
def issue_credit_note_from_commercial_invoice(
    invoice_id: int,
    organization_id: int,
    *,
    reason: str = 'Devolución total',
) -> ElectronicInvoiceDocument:
    """Emite NCR electrónica referenciada a la FE aceptada de la factura comercial."""
    if not is_efactura_enabled_for_org(organization_id):
        raise ValueError('Módulo efactura no habilitado para esta organización.')
    inv = Invoice.query.filter_by(id=int(invoice_id), organization_id=int(organization_id)).first()
    if not inv:
    ...
    if parent_fe is None or not (parent_fe.cufe or '').strip():
        raise ValueError('No hay factura electrónica aceptada con CUFE para emitir NCR.')
    existing_ncr = (
        ElectronicInvoiceDocument.query.filter(
            ElectronicInvoiceDocument.organization_id == int(organization_id),
    ...
    if existing_ncr is not None:
        raise ValueError(f'Ya existe una NCR activa para esta factura (documento #{existing_ncr.id}).')
    ...
    config = cfg_svc.get_or_create_provider_config(organization_id)
    if not cfg_svc.config_ready(config):
        raise ValueError('Configure y active el proveedor FE antes de emitir NCR.')

    from nodeone.modules.efactura.services.mapper import build_credit_note_payload

    pac_payload = build_credit_note_payload(
        config,
        inv,
        lines,
        contact,
    ...
    doc = ElectronicInvoiceDocument(
        ...
        environment=config.environment,
        document_type='credit_note',
        internal_reference=f'NCR-{inv.number}'[:120],
        source_model='invoice',
        source_id=int(inv.id),
        customer_name=fiscal_display_name(contact) if isinstance(contact, Contact) else display_name(contact),
        customer_tax_id=(getattr(contact, 'tax_id', None) or '').strip() or None,
    ...
    _log_event(
        ...
        'emit_credit_note',
        document_id=doc.id,
        message=f'NCR factura {inv.number}',
        request_payload=pac_payload,
    )
    try:
        result = adapter.emit_credit_note(doc, pac_payload)
    except Exception as exc:
        doc.status = 'error'
        doc.error_message = str(exc)
        _log_event(organization_id, 'error', document_id=doc.id, message=str(exc))
