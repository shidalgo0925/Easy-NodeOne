#!/usr/bin/env python3
"""
Eventos: miembros, admin y API pública.

Ubicación modular: nodeone.modules.events.routes
Compat: event_routes.py reexporta estos blueprints.
"""
...
def _users_for_event_admin_pickers():
...
def _current_org_id_for_events():
    ...
    try:
        if current_user.is_authenticated and not current_user.is_admin:
            return int(getattr(current_user, 'organization_id', 1) or 1)
    except Exception:
        pass
    try:
...
def confirm_event_registration(event_id, registration_id):
    ...
    try:
        ...
        email_sent = email_service.send_email(
            subject=f'[Easy Thesis] Registro confirmado: {event.title}',
...
def admin_events_index():
    ...
    stats = {
        ...
        'published': scoped_base.filter(Event.publish_status == 'published').count(),
...
def duplicate_event(event_id):
    """Duplicar evento"""
    ensure_models()
    original = _scoped_events_query().filter(Event.id == event_id).first_or_404()
