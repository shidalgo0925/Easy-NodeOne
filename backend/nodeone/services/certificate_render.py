"""Motor render plantilla JSON → HTML → PDF (certificados membresía y eventos)."""
...
def abs_certificate_url(base: str, url: str | None) -> str:
    if not url:
        return ''
    u = (url or '').strip()
...
def render_html_from_json_layout(
    template_model,
    data,
    base_url,
    qr_base64=None,
    ...
    use_file_urls: True para WeasyPrint (file://); False para vista previa en navegador.
    """
    ...
    if bg_url and base_url:
        bg_url = abs_certificate_url(base_url, bg_url)
        ...
        if el_type == 'text':
            ...
            if name in IMAGE_CERT_VARIABLES:
                ...
                if src and base_url and not src.startswith('data:'):
                    src = abs_certificate_url(base_url, src)
