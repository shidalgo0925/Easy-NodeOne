...
class TestProductLandingContent(unittest.TestCase):
    def test_eposone_has_full_sections(self):
        ...
        self.assertEqual(c['plans'][0]['name'], 'Standalone')
        self.assertIn('15.00', c['plans'][0]['price'])
        self.assertEqual(c['plans'][1]['name'], 'Starter')
        self.assertIn('29.95', c['plans'][1]['price'])
        self.assertEqual(c['plans'][2]['name'], 'Business')
        self.assertIn('39.95', c['plans'][2]['price'])
        self.assertEqual(c['plans'][3]['name'], 'Enterprise')
        self.assertIn('79.95', c['plans'][3]['price'])
        self.assertEqual(len(c['plan_compare']['headers']), 5)
        self.assertIn('39.95', c['plan_compare']['rows'][0][3])
