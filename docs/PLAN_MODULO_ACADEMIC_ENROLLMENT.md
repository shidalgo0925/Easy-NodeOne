...
- **Admin** listado + CRUD: `GET /admin/academic-enrollment/programs`, `.../programs/new`, `.../programs/<id>/edit` (programa + planes: alta, editar, activar, eliminar; archivar/eliminar programa).
- **Menú** Educación → «Programas inscripción»; **Productos** → «Catálogo formativo» (misma ruta).
