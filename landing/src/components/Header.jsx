...
// URL de la app (Flask). Producción: VITE_APP_URL=https://appprd.easynodeone.com (ver .env.production)
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-gray-light bg-white/95 backdrop-blur-sm">
      <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-2 px-4 py-2">
        <a href="/" className="flex items-center shrink-0" aria-label="EasyNodeOne inicio">
        ...
        <nav className="hidden items-center gap-4 lg:flex lg:gap-5">
          {nav.map((item) => (
            <a
              key={item.label}
              href={item.href}
              className="text-sm font-bold text-gray-mid hover:text-secondary xl:text-base"
            >
              {item.label}
            </a>
          ))}
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="inline-flex items-center justify-center rounded-lg p-2 text-gray-mid hover:bg-gray-light lg:hidden"
      ...
      {mobileOpen && (
        <div className="border-t border-gray-light bg-white px-4 py-4 lg:hidden">
