#!/usr/bin/env bash
# Despliega nginx-moodle-cursos.conf (Moodle en cursos.easynodeone.com). Requiere sudo.
set -euo pipefail

SITE_NAME="${NGINX_MOODLE_SITE_NAME:-cursos.easynodeone.com}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
SRC="${REPO_ROOT}/nginx-moodle-cursos.conf"
AVAILABLE="/etc/nginx/sites-available/${SITE_NAME}"
ENABLED="/etc/nginx/sites-enabled/${SITE_NAME}"
BACKUP_ROOT="/var/backups/nginx-moodle-cursos"
TS="$(date +%Y%m%d_%H%M%S)"
DRY_RUN=0

usage() {
  echo "Uso: sudo $0 [--dry-run]"
  echo "  --dry-run  Solo muestra acciones."
  echo "Env: NGINX_MOODLE_SITE_NAME (default: cursos.easynodeone.com)"
}
...
echo "OK: nginx recargado (Moodle → cursos.easynodeone.com)."
